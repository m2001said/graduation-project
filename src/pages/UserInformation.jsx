    ...
    <div className="demo-section bg-gradient-to-r from-[#481b91] to-[#240d55] min-h-screen">
      <div className="container flex flex-col px-4 py-10 mx-auto items-center">
        ...
        <div className="img-box flex flex-col md:flex-row items-center gap-4 mb-8">
          ...
          <label
            htmlFor="file-upload"
            className="user_img_upload text-white cursor-pointer"
          >
            Change Image
          </label>
          {selectedFile && (
            <button
              ...
              className="ml-4 bg-gray-500 px-2 py-1 rounded text-white"
            >
              Upload
            </button>
          )}
        ...
        <div className="flex flex-col md:flex-row my-4 justify-between gap-4 items-center w-1/2">
          ...
          <input type="email" id="email" className="p-2 outline-none md:w-3/4rounded border-solid border-1 border-white bg-white text-black" value={email} onChange={(e) => setEmail(e.target.value)} />
        </div>
  
        <div className="flex flex-col md:flex-row my-4 justify-between gap-4 items-center w-1/2">
          <label className="text-white" htmlFor="oldPassword">Old Password</label>
          <input type="password" id="oldPassword" className="p-2 outline-none md:w-3/4rounded border-solid border-1 border-white bg-white text-black" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} />
        </div>

        <div className="flex flex-col md:flex-row my-4 justify-between gap-4 items-center w-1/2">
          <label className="text-white" htmlFor="newPassword">New Password</label>
          <input type="password" id="newPassword" className="p-2 outline-none md:w-3/4rounded border-solid border-1 border-white bg-white text-black" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
        </div>
  
        <div className="flex flex-col md:flex-row my-4 justify-between gap-4 items-center w-1/2">
          <label className="text-white" htmlFor="username">User Name</label>
          <input type="text" id="username" className="p-2 outline-none md:w-3/4rounded border-solid border-1 border-white bg-white text-black" value={username} onChange={(e) => setUsername(e.target.value)} />
        </div>
  
        <button style={{ backgroundColor: "var(--color-3)" }} className="text-center my-4 rounded bg-gray-500 px-4 py-2 text-white w-1/2" onClick={handleUpdateInformation}>Update Information</button>
