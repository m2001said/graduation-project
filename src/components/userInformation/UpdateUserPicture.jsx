  ...
  const handleUpload = async (error, result) => {
    ...
    if (result.event === "success") {
      ...
      try {
        ...
        formData.append("avatar", blob, "avatar.jpg"); // append the blob with name "avatar"

        const backendResponse = await axios.post("https://websitebuilderbackend-production-716e.up.railway.app/upload", formData, {
          headers: {
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row my-4 justify-between gap-4 items-center w-full md:w-1/2 mx-auto">
        <div style={{ maxWidth: "350px", height: "300px", overflow: "hidden" }} className="rounded">
          <img className="object-cover w-full h-full" src={uploadedImage || "https://avatars.githubusercontent.com/u/127410127?v=4"} alt="Uploaded" />
        </div>
          ...
          {({ open }) => (
            <div className="w-full">
              <button
                ...
                className="text-center my-4 rounded bg-gray-500 px-4 py-2 text-white"
