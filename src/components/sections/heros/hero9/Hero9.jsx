    ...
    <main className="mb-12 pt-4">
          ...
          <form className="shadow-2xl rounded-3xl py-8 lg:w-1/3 relative lg:mt-0 mt-12 w-full">
            <div className="bg-[var(--website-9-color-3)] shadow-2xl rounded-3xl py-8">
              ...
              <div className="py-12 flex flex-col gap-8 px-4">
                <input
                  type="text"
                  placeholder={hero.form.inputPlaceholders.location}
                  className="h-16 outline-none rounded-3xl bg-[var(--website-9-color-1)] border border-[var(--website-9-color-1)] px-6"
                />
                <select
                  placeholder={hero.form.inputPlaceholders.propertyType}
                  className="h-16 outline-none rounded-3xl bg-[var(--website-9-color-1)] border border-[var(--website-9-color-1)] px-6"
                >
                  {hero.form.options.propertyType.map((option, index) => (
                    <option key={index}>{option}</option>
                  ))}
                </select>
                <select
                  placeholder={hero.form.inputPlaceholders.rooms}
                  className="h-16 outline-none rounded-3xl bg-[var(--website-9-color-1)] border border-[var(--website-9-color-1)] px-6"
                >
                  {hero.form.options.rooms.map((option, index) => (
                    <option key={index}>{option}</option>
                  ))}
                </select>
              </div>
      ...
      <div className="lg: container mx-auto px-3 py-8">
        <span className="lg:flex items-end gap-x-60 lg:text-left text-center">
          <button className="lg:block  bg-gradient-to-r from-[var(--website-9-color-6)] to-[var(--website-9-color-1)] rounded-full h-24 w-24 cursor-auto"> </button>
          <p className="text-[var(--website-9-color-4)] ">Trusted by 100+ companies across the global</p>
        </span>
        <div className="py-12">
