      ...
      <div className="xl:flex items-center container max-w-[1300px] mx-auto lg:px-3">
        <section className="xl:w-1/2 xl:text-left text-center">
          <p className="text-[25px] font-bold darkModeTextColor15">
            {hero.mainTitle.title} <span className="btnHoverColor15">{hero.mainTitle.name}</span>
          </p>
          <h1 className="xl:text-[65px] text-[74px] mt-10 mb-5 sm:py-0 py-4 btnHoverColor15 textShadow">{hero.mainTitle.role}</h1>
          ...
          <div className="xl:block hidden">
            ...
            <div className="flex items-center w-fit text-[13px] font-bold pt-4">
              <span className="block rounded-full h-14 border border-black border-solid shadowbtn">
                <button className="bg-transparenent darkModeTextColor15 h-full font-bold px-10 rounded-full hover:border-2 border-solid bg-[var(--website-15-color-3)]">
                  {hero.actionButton.name}
                </button>
              </span>
              <hr className="w-10 h-[3px] bg-black mr-6 btnHoverColor15" />
              <p className="btnHoverColor15">{hero.actionButton.description}</p>
            </div>
          </div>
        </section>
