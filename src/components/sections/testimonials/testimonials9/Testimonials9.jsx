  ...
  return (
    <main className="container mx-auto px-3 lg:flex justify-between pt-20 testimonial-9">
      <div className="lg:w-1/3 flex flex-col">
        ...
        <p className="text-[var(--primary-text-color-9)] text-sm">{block9.speach}.</p>
      ...
      <div className="relative rounded-2xl shadow-2xl lg:w-2/5 lg:p-16 p-6 lg:mt-0 mt-8">
        ...
        <p className="font-medium pt-2 text-[var(--primary-text-color-9)] pb-8">{currentTestimonial.quote}</p>
        <div className="border-t border-[var(--grey-9)] flex items-center py-4">
          ...
          <span className="lg:flex items-center justify-between w-full gap-x-4 font-medium pl-3">
            ...
            <div className="flex flex-row">
              {" "}
              {[...Array(5)].map((_, index) => (
                <button key={index} className="cursor-auto text-[var(--primary-next-9-color)] last:text-[var(--grey-9)]">
        ...
        <button className="bg-gradient-to-r from-[var(--icon-hover-color-9)] to-[var(--white-9)] rounded-full h-16 w-16 cursor-auto absolute -top-6 left-6"></button>
      </div>
    </main>
  );
};
