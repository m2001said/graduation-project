...
const Navbar15 = () => {
  ...
  const navbar = useSelector((state) => {
    if (pathname.includes("own-page")) {
      ...
      return state.template15.navbar;
    }
  });
  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center navbar nav15">
      <img src={navbar.logo} alt="logo" className="w-[124px] h-[32px]" />

      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navbar.links.map((nav, index) => (
          <li
            key={nav.title}
            className={`font-poppins font-normal cursor-pointer text-[16px] ${active === nav.title ? "var(--website-15-color-2)" : "var(--website-15-color-3)"} ${index === navbar.links.length - 1 ? "mr-10" : "mr-10"
              }`}
        ...
        <div className={`${!toggle ? "hidden" : "flex"} p-6 bg-black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] rounded-xl sidebar`}>
          <ul className="list-none flex justify-end items-start flex-1 flex-col">
            {navbar.links.map((nav, index) => (
              <li
                key={nav.title}
                className={`font-poppins font-medium cursor-pointer text-[16px] ${active === nav.title ? "var(--website-15-color-4)" : "var(--website-15-color-6)"} ${index === navbar.links.length - 1 ? "mb-0" : "mb-4"
                  }`}
                onClick={() => setActive(nav.title)}
              >
                <a href={nav.url} style={{ color: active === nav.title ? "var(--website-15-color-3)" : "var(--website-15-color-1)" }}>
                  {nav.title}
                </a>
              </li>
            ))}
