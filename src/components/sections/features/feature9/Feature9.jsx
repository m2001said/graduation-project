...
const Feature9 = () => {
  const features = useSelector((state) => state.template9.features);
  ...
  return (
    <main className=" justify-center  bg-gradient-to-t from-[var(--website-9-color-3)] to-[var(--website-9-color-1)] feature-9">
      ...
      <section className="grid md:grid-cols-2 grid-cols-1 md:gap-3 gap-4 pb-12 section-9 justify-center md:text-left ">
        {features.features.map((neighborhood) => (
          <div key={neighborhood.id} className="relative md:col-span-3">
            <img src={neighborhood.imgUrl} alt="" className="rounded-3xl object-cover md:h-80 h-40 w-full" />
            <span className="absolute md:bottom-8 bottom-4 md:left-8 left-4">
              <p className="text-[var(--website-9-color-1)] md:text-5xl text-xl font-semibold md:pb-3">{neighborhood.properties}</p>
