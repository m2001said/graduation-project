...
const Footer7 = () => {
  ...
  const socialMediaIcons = Object.values(socialMedia).slice(1);

  return (
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`} style={{ color: "var(--color-dim-white)" }}>
        ...
        <div className="flex-[1.5] w-full flex flex-row justify-between flex-wrap md:mt-0 mt-10">
          {footer.links.map((section, index) => (
            <div key={index} className={`flex flex-col ss:my-0 my-4 min-w-[150px]`}>
              <h4 className="font-poppins font-medium text-[18px] leading-[27px] text-white">{section.title}</h4>
              <ul className="list-none mt-4">
                {section.links.map((link, index) => (
                  <li
                    ...
                    onMouseLeave={(e) => (e.target.style.color = "var(--color-dim-white)")}
                  >
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]">
        <p className="font-poppins font-normal text-center text-[18px] leading-[27px] text-white">{socialMedia.title}</p>
