...
const Footer13 = ({ template }) => {
  ...
  return (
    <footer  id={footer.sectionId} className="bg-[var(--website-13-color-1)] text-[var(--website-13-color-3)] pb-16">
            ...
            <ul className="lg:flex items-center justify-between">
              {footer.footerSections.map((section, index) => (
                <li key={index} className="lg:w-1/4 lg:pb-0 pb-3">
                  <p className="font-bold"><a href={section.url} target="_blank" rel="noreferrer" >{section.title}</a></p>
                  <ul>
