      ...
      <div className="xl:w-[1200px] max-w-[75%] mx-auto px-3 pt-8">
        <section className="relative lg:py-0 py-8">
          ...
          <div className="md:flex items-center justify-between lg:pt-20 pt-8 gap-x-16">
            {projects.project && projects.project.map((project, index) => <Cards key={index} img={project.img} title={project.title} description={project.description} icon={project.icon} />)}
          </div>
        <div className="flex justify-center gap-2 flex-wrap ">
          {projects.buttons.map((button, index) => ( 
          <button key={index} className="rounded-full mx-auto mt-12 px-4 py-2 items-center text-sm bg-[var(--website-13-color-4)] text-[var(--website-13-color-1)] gap-2 transition ease-out duration-300 hover:scale-110">
          ...
          {[1, 2, 3, 4].map((_, index) => (
            <img key={index} src={projects.icon} alt="arrow up" className="text-[var(--website-13-color-4)] text-4xl absolute top-0 left-0 rotate-[315deg] w-8" />
          ))}
        </section>
      </div>
    </div>
