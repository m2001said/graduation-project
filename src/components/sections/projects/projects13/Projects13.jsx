    ...
    <div id="projects13" className="relative overflow-hidden bg-[var(--fff6ed)] lg:py-20 py-8">
      ...
      <div className="xl:w-[1200px] max-w-[75%] mx-auto px-3 pt-8">
        <section className="relative lg:py-0 py-8 ">
          ...
          <button className="rounded-full mx-auto mt-12 px-4 py-2 items-center text-sm bg-[#fd7b747] text-[var(--color-darkG-13)] gap-2 transition ease-out duration-300 hover:scale-110 ">
            <p className="text-[var(--color-13-yel)]">Load more</p>
            <img src={projects.load} className="text-xl w-8" />
          </button>
          {[1, 2, 3, 4].map((_, index) => (
            <img src={projects.arrup} key={index} className="text-[var(--color-13-yel)] text-4xl absolute top-0  left-0 rotate-[315deg] w-8" />
          ))}
        </section>
        <section className="flex items-center justify-between mt-24 mb-16 gap-2">
          <span className="flex items-center  flex-col gap-x-2">
            <img src={projects.logo1} className="w-6 text-[var(--color-darkG-13)] text-3xl" />
            <p className="font-medium primary-font text-xl text-[var(--color-darkG-13)]">{projects.logo1talk}</p>
          </span>
          <span className="flex items-center  flex-col gap-x-2">
            <img src={projects.logo2} className="w-6 text-[var(--color-darkG-13)] text-3xl" />
            <p className="font-medium primary-font text-xl text-[var(--color-darkG-13)]">{projects.logo2talk}</p>
          </span>
          <span className="flex items-center  flex-col gap-x-2">
            <img src={projects.logo3} className="w-6 text-[var(--color-darkG-13)] text-3xl" />
            <p className="font-medium primary-font text-xl text-[var(--color-darkG-13)]">{projects.logo3talk}</p>
          </span>
          <span className="flex items-center  flex-col gap-x-2">
            <img src={projects.logo4} className="w-6 text-[var(--color-darkG-13)] text-3xl" />
            <p className="font-medium primary-font text-xl text-[var(--color-darkG-13)]">{projects.logo4talk}</p>
          </span>
          <span className="flex items-center  flex-col gap-x-2">
            <img src={projects.logo5} className="w-6 text-[var(--color-darkG-13)] text-3xl" />
            <p className="font-medium primary-font text-xl text-[var(--color-darkG-13)]">{projects.logo5talk}</p>
          </span>
        </section>
        <section className="lg:flex justify-between items-center relative mt-12 lg:p-8">
          <div className="lg:w-1/2">
            <p className="text-[var(--color-13-yel)]">{projects.OurNewsletter}</p>
            ...
            <p className="text-[var(--color-darkG-13)] leading-8">{projects.subsdesc}</p>
            <div className="relative flex items-center pt-8">
              ...
              <button className="absolute md:right-4 right-1 rounded-full md:px-8 flex items-center md:text-sm text-xs bg-[var(--color-13-yel)] text-[var(--color-darkG-13)] gap-2 transition ease-out duration-300 transform hover:scale-110">
                <p className="text-[var(--color-darkG-13)]">{projects.Subs}</p>
