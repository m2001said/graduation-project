    ...
    <main id={team.sectionId} className="container mx-auto px-3 pt-8 pb-12 team9">
      ...
      <section className="lg:flex items-center grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 lg:gap-12  gap-8 justify-center">
        {team.members.map((member) => (
          <div
            key={member.id}
            className="w-full bg-[var(--website-9-color-1)] rounded-lg shadow-lg overflow-hidden transition-transform transform hover:-translate-y-2 hover:shadow-2xl shadow-[var(--website-9-color-3)] hover:shadow-[var(--website-9-color-5)]"
          >
            <img
              src={member.imgUrl}
              alt={member.name}
